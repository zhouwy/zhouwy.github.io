...
function getRank(){
    ...
    for(var n=0;n<form.elements.length;n++){ 0="" p="Math.floor(n/5);" if(p="=0){" problem[p]+="form.elements[n].flag;" }else="" }="" for(var="" x="0;x<7;x++){" if(problem[x]="=0){" var="" y="x+1;" alert("问卷问题"+y+"未填写");="" result="parts.map(function(item,index,array){" *console.log(item);*="" switch="" (item){="" case="" "a"="" :="" return="" item="10;" "b"="" "c"="" "d"="" "e"="" });="" for="" (var="" i="0;" <="" result.length;="" i++)="" {="" score+="parseInt(result[i]);" };="" score;="" questionbtn.addeventlistener("click",function(event){="" event.preventdefault();="" score="getRank();" show_tip.style.display="block" ;="" if="" (="" &&="" html="" span="得分：" +score+";保守型——低风险级别产品";="" +="<span class='user-risk-span'>" "<\="">";
        ...
        html +="<p class="user-risk-p">" + p + "<\ p="">";
        result.innerHTML = html;
    } else if(30 < score && score <= 45){="" var="" html="" ;="" span="得分：" +score+";平衡型——中等风险级别产品";="" p="您有一定的风险承受能力，对投资收益比较敏感，期望通过长期且持续的投资获得高于平均水平的回报，通常更注重十年甚至更长期限内的平均收益。所以中等风险收益的投资品种比较适合您，回避风险的同时有一定的收益保证。" +="<span class='user-risk-span'>" "<\="">";
        html +="<p class="user-risk-p">" + p + "<\ p="">";
        result.innerHTML = html;
    } else if(45 < score && score <=60){ var="" html="" ;="" span="得分：" +score+";积极型——较高风险级别产品";="" p="您有中高的风险承受能力，愿意承担可预见的投资风险去获取更多的收益，一般倾向于进行中短期投资。所以中高等级的风险收益投资品种比较适合您，以一定的可预见风险换取超额收益。" +="<span class='user-risk-span'>" "<\="">";
        html +="<p class="user-risk-p">" + p + "<\ p="">";
        result.innerHTML = html;
    } else if(60<score &&="" score="" <70){="" var="" html="" ;="" span="得分：" +score+";激进型——高风险级别产品";="" p="您有较高的风险承受能力，是富有冒险精神的积极型选手。在投资收益波动的情况下，仍然保持积极进取的投资理念。短期内投资收益的下跌被您视为加注投资的利好机会。您适合从事灵活、风险与报酬都比较高的投资，不过要注意不要因一时的高报酬获利而将全部资金投入高风险操作，务必做好风险管理与资金调配工作。" +="<span class='user-risk-span'>" "<\="">";
        html +="<p class="user-risk-p">" + p + "<\ p="">";
        result.innerHTML = html;
    }
},false);
...
function getRankscore(value) {
    var getRank_score;
    if(value<=15){ getrank_score="0.9;" }else="" if(value="">15&&value<=30){ getrank_score="0.7;" }else="" if(value="">30&&value<=45){ getrank_score="0.5;" }else="" if(value="">45&&value<=60){ getrank_score="0.3;" }else="" if(value="">60){
        getRank_score= 0.1;
    }
...
submit.onclick=function () {
        ...
        function Array_2(nRow,nColumn){
            ...
            for(i=0;i<nrow;i++){ 将每一个子元素又定义为数组="" array1[i]="new" array();="" ----------------------------------------="" for(n="0;n<nColumn;n++){" array1[i][n]="" ;="" 此时aa[i][n]可以看作是一个二级数组="" }="" --------------------------------------="" return="" array1;="" var="" array_2="Array_2(23,2);" total="0;" function="" check()="" {="" for(var="" s="0;s<7;s++){" if(total="=7&&radiopre_score!=1&&durationpre_score!=1&&acceptance_levelpre_score!=1){" i="0;i<platform.length;i++){" platform[i].totalscore="0;" j="0;j<4;j++){" platform[i].totalscore+="platform[i].vector[j]*arr[j];" *console.log(parsefloat(platform[i].totalscore));*="" array_2[i][0]="platform[i].totalscore;" array_2[i][1]="platform[i].platname;" k="0;k<23;k++){" l="0;l<23-k-1;l++){" if(array_2[l][0]<array_2[l+1][0])="" temp="array_2[l][0];" array_2[l][0]="array_2[l+1][0];" array_2[l+1][0]="temp;" temp2="array_2[l][1];" array_2[l][1]="array_2[l+1][1];" array_2[l+1][1]="temp2;" html+="<li>" +"<span="">"+"TOP"+j+""+"<span>"+array_2[i][1]+"</span>"+"<span>"+array_2[i][0].toFixed(3)+"</span>"+"";
            }
...
</nrow;i++){></=60){></=45){></=30){></=15){></\></p></score></\></p></=60){></\></p></=></\></p></=></\></p></form.elements.length;n++){>